    ...
    <div style={{ minHeight: "100vh", background: C.charcoal }}>
      {session?.projectId ? (
        <ClientDashboardContent
          ...
          dashboardTitle={session.dashboardTitle}
...
function ClientDashboardContent({ projectId, accessToken, clientName, dashboardTitle, onChangePassword, onLogout }: {
  projectId: number; accessToken: string; clientName: string; dashboardTitle: string | null;
  ...
  const respondMutation = trpc.clientPortal.respondToAction.useMutation({
    onSuccess: () => {
      // Refetch data after responding
    },
  });
